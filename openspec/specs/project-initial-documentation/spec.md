...
### Requirement: Cada projeto possui AGENTS.md com contexto para IA
Todos os 22 projetos SHALL ter um arquivo `AGENTS.md` na raiz do submodule contendo: propósito (2-3 frases), o que este projeto NÃO faz, e pontos de integração principais.

#### Scenario: AGENTS.md presente em todos os projetos
- **WHEN** se lista a raiz de cada um dos 22 submodules
- **THEN** existe um arquivo `AGENTS.md` em cada um

#### Scenario: AGENTS.md define limites do contexto
- **WHEN** se lê o `AGENTS.md` de qualquer projeto
...
### Requirement: AGENTS.md dos submodules referencia o AGENTS.md root como template
Os `AGENTS.md` criados em cada submodule (escopo de mudança futura) SHALL seguir a estrutura estabelecida no `AGENTS.md` root do monorepo.

#### Scenario: Estrutura compatível com o template root
- **WHEN** se cria um `AGENTS.md` em qualquer submodule
- **THEN** as seções usam os mesmos nomes e ordem definidos no `AGENTS.md` root
...
### Requirement: AGENTS.md dos submodules é agnóstico de ferramenta de IA
Os `AGENTS.md` de cada submodule SHALL seguir o mesmo requisito de agnosticismo do `AGENTS.md` root.

#### Scenario: Sem referências a ferramentas específicas
- **WHEN** se lê o `AGENTS.md` de qualquer submodule
