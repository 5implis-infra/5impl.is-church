...
description: Security audit for Claude skills before installation. Run this whenever a user wants to install, review, or validate a new skill for safety and security vulnerabilities. Use as `/skill-security-auditor <path/to/skill-directory>`. ALWAYS trigger this skill when someone says: "audit this skill", "is this skill safe?", "check a skill for security issues", "review skill before installing", "validate skill security", or when the user is about to install an unfamiliar skill. Also trigger proactively when a new skill directory is mentioned alongside words like "install", "add", or "use".
---
...
```
/skill-security-auditor <skill-directory>
```
...
find ~/.claude/skills ~/.codex/skills .claude/skills .agents/skills \
  -name "scan.py" -path "*/skill-security-auditor/*" 2>/dev/null | head -1
```
...
### Step 6. After PASS / APPROVE

The skill is safe to activate. Stamp it as audited and move it to the active directory:

```bash
touch <skill-directory>/.audited
mv <skill-directory> .agents/skills/<skill-name>
```

The `.audited` marker prevents the quarantine hook from re-triggering on this skill in future scans.

Then inform the user the skill is now active.

### 7. After FAIL / REJECT

List every critical issue. Do not enable the skill. If the skill is already in the active directory, quarantine it:

```bash
mv .agents/skills/<skill-name> .agents/skills/disabled/<skill-name>
```

Suggest the user contact the skill's author or remove it entirely.
